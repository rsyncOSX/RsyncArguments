...
public struct SSHConfiguration {
    ...
    public init(portString: String?, identityFile: String?) {
        if let portString, portString != "-1", let port = Int(portString) {
            self.port = port
        } else {
            port = nil
        }
